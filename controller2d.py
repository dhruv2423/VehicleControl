...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        self.vars.create_var('prev_int',0.0)
        ...
        if self._start_control_loop:
            ...
            v_error = v_desired - v
            I = self.vars.prev_int + v_error
            D = v_error - self.vars.prev_error
            throttle_output = kp*v_error + ki*I*dt + kd*D/dt
            ...
            k_e = 0.3
            k_s = 10
            present_xy = np.array([x,y])
            crosstrack_error = np.min(np.sum((present_xy - np.array(waypoints)[:, :2])**2, axis=1))
            crosstrack_error = np.min(np.sum((present_xy - np.array(waypoints)[:, :2])**2, axis=1))
            ...
            yaw_diff_crosstrack = np.arctan(k_e * crosstrack_error / (k_s + v))
